...
import com.iscoderweather.app.service.AutoUpdateService;
...
public class WeatherActivity extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!TextUtils.isEmpty(countyCode)) {//判断县级代号是否为空
			publishText.setText("同步中...");
			weatherInfoLayout.setVisibility(View.INVISIBLE);
			queryWeatherCode(countyCode);//不为空就去查询天气。
		} else {
			showWeather();//若为空就直接显示本地天气。
		}
	}
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener () {
			...
			public void onFinish(String response) {
				if ("countyCode".equals(type)) {
					...
					//处理服务器返回的天气信息
					Utility.handleWeatherResponse(WeatherActivity.this, response);//因为在子线程中运行这里Context写this不行。
